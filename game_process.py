...
def pick_word(words_list, round_num):
...
def run_game(difficulty="easy"):
    ...
    # ✅ Difficulty settings (set ONCE, not inside the loop)
    settings = {
        "easy":   {"lives": 3, "timer_start": 8.0, "timer_min": 3.0, "timer_drop": 0.25},
        "medium": {"lives": 3, "timer_start": 6.5, "timer_min": 2.5, "timer_drop": 0.35},
        "hard":   {"lives": 2, "timer_start": 5.5, "timer_min": 2.0, "timer_drop": 0.45},
    }
    cfg = settings.get(difficulty, settings["easy"])

    player_lives = cfg["lives"]
    score = 0
    timer_length = cfg["timer_start"]
    typed = ""
    rounds = 0
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if game_over:
                    if event.key == pygame.K_RETURN or event.key == pygame.K_SPACE:
                        # reset with the same difficulty config
                        player_lives = cfg["lives"]
                        score = 0
                        timer_length = cfg["timer_start"]
                        typed = ""
